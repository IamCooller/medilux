...
class ModelCatalogProduct extends Model {
	...
	public function getProductDescription($product_id){
		...
		if ($query->num_rows) {
			return array(		
				...
				'performance_capability'      => $query->row['performance_capability'],
				'product_video' => $query->row['product_video'],	
			);
		} else {
			return false;
		}
	...
	public function getProduct($product_id) {
		...
		if ($query->num_rows) {
			return array(
				...
				'product_discond'           => $query->row['product_discond']
			);
		} else {
			return false;
		}
	...
	public function getProducts($data = array()) {
		$sql = "SELECT p.*, p.product_discond, pd.*, (SELECT AVG(rating) AS total FROM " . DB_PREFIX . "review r1 WHERE r1.product_id = p.product_id AND r1.status = '1' GROUP BY r1.product_id) AS rating, (SELECT price FROM " . DB_PREFIX . "product_discount pd2 WHERE pd2.product_id = p.product_id AND pd2.customer_group_id = '" . (int)$this->config->get('config_customer_group_id') . "' AND pd2.quantity = '1' AND ((pd2.date_start = '0000-00-00' OR pd2.date_start < NOW()) AND (pd2.date_end = '0000-00-00' OR pd2.date_end > NOW())) ORDER BY pd2.priority ASC, pd2.price ASC LIMIT 1) AS discount, (SELECT price FROM " . DB_PREFIX . "product_special ps WHERE ps.product_id = p.product_id AND ps.customer_group_id = '" . (int)$this->config->get('config_customer_group_id') . "' AND ((ps.date_start = '0000-00-00' OR ps.date_start < NOW()) AND (ps.date_end = '0000-00-00' OR ps.date_end > NOW())) ORDER BY ps.priority ASC, ps.price ASC LIMIT 1) AS special";
	...
	public function getSpecialProducts(){
		$sql = "SELECT * FROM " . DB_PREFIX . "product as p LEFT JOIN " . DB_PREFIX . "product_description as pd ON (p.product_id = pd.product_id) WHERE pd.language_id = '" . (int)$this->config->get('config_language_id') . "' AND p.product_offer = 1 ORDER BY p.product_id DESC LIMIT 4";
		$query = $this->db->query($sql);
